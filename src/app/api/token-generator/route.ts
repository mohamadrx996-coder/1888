// src/app/api/token-generator/route.ts - Token Generator v8.0 - Smart Discord Format
...
// =====================================
// Example: MTQ5NzIwNjE2NjQ0OTgxNTY0Mg.GstP6D.fPgE4oS2F_Eod_8CuMV95kcuACRy0HKsiy2YMQ (72 chars)
//
// P1 = Base64URL(UserID) = 26 chars (19-digit snowflake)
// P2 = Base64URL(Timestamp) = 6 chars
// P3 = Base64URL(HMAC-SHA256) = 38 chars
// Total = 72 chars (with 2 dots)
//
// Older tokens (pre-2022): P1=24 chars (17-18 digit snowflake) → 70 chars total
// All valid ranges: P1=24-26, P2=6, P3=38
...
function analyzeToken(token: string): TokenAnalysis {
  ...
  // Discord token validation ranges
  // P1: 24-26 chars | P2: 6 chars | P3: 38 chars
...
// P1: 26 chars (base64 of 19-digit snowflake) - same as real Discord tokens
function genP1(uid?: string): string {
  if (uid) return stringToBase64Url(uid);
  let snow = genSnowflake();
  let p1 = stringToBase64Url(snow);
  // Modern snowflakes produce 26-char P1. Loop until we get 24-26.
  let attempts = 0;
...
async function genP3Smart(): Promise<string> {
  ...
  // 28 bytes → exactly 38 base64url chars, pad or trim to be safe
  if (result.length >= 38) return result.substring(0, 38);
  // Append random chars if somehow shorter (very rare)
  const extra = uint8ToBase64Url(getRandomBytes(6));
  return (result + extra).substring(0, 38);
}

// Generate complete Discord-style token (72 chars)
...
function analyzeFragment(fragment: string): FragmentAnalysis {
  ...
  // Check if it's a full token (3 parts with dots)
  ...
  try {
    ...
    if (action === 'generate') {
      ...
      if (userId && /^\d{17,20}$/.test(userId.trim())) {
        ...
        // Fragment mode - smart completion
        const fragAnalysis = analyzeFragment(fragment);
        ...
        // Random mode - full Discord format
        genFn = generateToken;
      }
