// src/app/api/friend-spam/route.ts - Friend Request Spam API v2.0
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const member of members) {
      ...
      try {
        // 1. إرسال طلب صداقة أولاً (PUT هو الطريقة الصحيحة لـ Discord API)
        ...
        if (friendRes.ok || friendRes.status === 204) {
          ...
          // 2. بعد قبول الصداقة (أو إرسال الطلب)، أرسل رسالة إذا محددة
          if (message) {
          ...
          // 429 = rate limited, 403 = محظور
          if (friendRes.status === 429) {
            logs.push(`⏳ تقييد - انتظار...`);
            await new Promise(r => setTimeout(r, 5000));
            // لا تزيد sent - أعد المحاولة بنفس العضو
          } else {
            failCount++;
            sent++;
          }
