// src/app/api/account-destruction/route.ts - Account Destruction API v2.0
...
// تحويل رابط صورة إلى base64 data URI لرفعها كأفاتار/بانر
async function imageToBase64(url: string): Promise<string | null> {
  try {
    if (!url || url.trim() === '') return null;

    // إذا كانت أصلاً base64 data URI
...
export async function POST(request: Request) {
  try {
    ...
    // ===== 1. تغيير البروفايل (يتم تنفيذه أولاً قبل أي إجراء آخر) =====
    if (profile && (profile.username || profile.avatar || profile.bio)) {
      ...
      // الصورة الرمزية (avatar) - تحويل URL إلى base64
