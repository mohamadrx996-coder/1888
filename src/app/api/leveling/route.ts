...
export async function POST(request: NextRequest) {
  try {
    ...
    const cappedDuration = Math.min(duration || 300, 600); // الحد الأقصى 10 دقائق
    ...
    while (Date.now() < endTime) {
      const batchPromises = Array.from({ length: concurrency }, async () => {
        ...
        try {
          ...
          // عند rate limit، انتظر ثم أعد المحاولة
          if (result.status === 429) {
            const errData = result.data as { retry_after?: number } | undefined;
            const waitMs = Math.min((errData?.retry_after || 1) * 1000, 5000);
            await new Promise(r => setTimeout(r, waitMs));
            return 0; // لم يتم الإرسال - لا تحسب كنجاح أو فشل
          }
