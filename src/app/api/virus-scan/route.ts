...
// ══════════════════════════════════════════════════════════════════
// TRJ Smart Pattern Engine v2 - محرك أنماط ذكي
// فقط الأنماط الخبيثة الحقيقية - لا إنذارات كاذبة
// ══════════════════════════════════════════════════════════════════

// أنماط خبيثة عالية الثقة فقط - هذه لا تظهر في كود عادي
...
// أنماط عادية مشتركة - لا يجب أن تسبب إنذار بمفردها
...
// ══════════════════════════════════════════════════════════════════
// TRJ Pattern Engine v2 - محرك أنماط ذكي ومتزن
// ══════════════════════════════════════════════════════════════════
function patternEngineAnalyze(content: string, fileName: string): { findings: number; threats: string[]; data: any; scanTime: number } {
  ...
  // فحص التركيبات الخطيرة (مزيج أنماط مشبوهة) - فقط هذه تعتبر مؤشر قوي
  ...
  // فقط التركيبات التالية تعتبر تهديد حقيقي
...
// ══════════════════════════════════════════════════════════════════
// TRJ AI Engine v2 - محرك ذكاء اصطناعي متقدم
// المحرك الأساسي - قراره هو الأهم
// ══════════════════════════════════════════════════════════════════
...
رد بتنسيق JSON فقط:
{
  ...
  "reasoning": "شرح مختصر لسبب الحكم - لماذا نظيف أو لماذا خبيث"
    ...
    return { findings: [], threat_assessment: 'unknown', scan_time: Math.round(performance.now() - startTime), confidence: 'none', reasoning: 'فشل الاتصال بمحرك AI' }
  }
}

// ══════════════════════════════════════════════════════════════════
// TRJ Heuristic Engine v2 - محرك استقرائي ذكي
// ══════════════════════════════════════════════════════════════════
function heuristicAnalyzeContent(content: string, fileName: string): { findings: number; threats: string[]; scan_time: number } {
  ...
  // 1. ملفات مع امتدادات مزدوجة (تقنية إخفاء معروفة)
  ...
  // 2. أسطر طويلة جداً (كود مبهم غالباً) - لكن فقط إذا كانت كثيرة
  ...
  // 5. تشفير + تنفيذ (فقط التركيبة الخطيرة)
  ...
  // 6. عدد كبير جداً من السلاسل المشفرة (فقط إذا كان أمراً غير طبيعي)
  ...
  // 7. اسم دوال مشبوه جداً (ليس مجرد أسماء غريبة)
...
// ══════════════════════════════════════════════════════════════════
// دمج النتائج - AI هو الحكم الأساسي
// ══════════════════════════════════════════════════════════════════
function combineResults(
  ...
  // ═══ حساب النتيجة - النظام الجديد ═══
  // AI هو الحكم الأساسي (60% من الدرجة)
  // Pattern engine ثانوي (30%)
  // Heuristic مساعد (10%)

  let aiScore = 0
  const aiScoreMap: Record<string, number> = { critical: 60, high: 45, medium: 25, low: 10, clean: 0, unknown: 0 }
  aiScore = aiScoreMap[aiResult.threat_assessment] || 0

  // إذا كان AI واثق من نظافة الملف - خفض الدرجة كثيراً
  if (aiResult.threat_assessment === 'clean' && aiResult.confidence === 'high') {
    aiScore = 0
  }

  let patternScore = 0
  patternScore += Math.min(dangerCount * 8, 30)  // أنماط خطيرة
  patternScore += Math.min(warningCount * 2, 10) // تحذيرات خفيفة

  let heuristicScore = Math.min(heuristicResult.findings * 3, 10)

  // درجة نهائية
  let score = Math.round(aiScore * 0.6 + patternScore * 0.3 + heuristicScore * 0.1)
  score = Math.min(score, 100)

  // ═══ حسم مستوى التهديد ═══
  // النظام الجديد يتطلب درجة عالية لاعتبار الملف مشبوه
  ...
  // عدد المحركات التي كشفت تهديدات
