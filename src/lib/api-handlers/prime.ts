...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'purchase') {
      ...
      if (!token || !key) {
        return NextResponse.json({ success: false, error: 'أدخل التوكن والمفتاح' });
      }
      ...
      if (!verifyResult.ok || !verifyResult.data) {
        return NextResponse.json({ success: false, error: 'توكن غير صالح', isPrime: false });
      }
      ...
      if (!buyerId) {
        return NextResponse.json({ success: false, error: 'أدخل ID المشتري' });
      }
